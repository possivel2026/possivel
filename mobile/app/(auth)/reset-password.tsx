...
export default function ResetPasswordScreen() {
  ...
  async function submit() {
    ...
    } catch (error) { Alert.alert('Erro', getErrorMessage(error)); } finally { setLoading(false); }
  }
  return <Screen scroll><Text style={styles.title}>Nova senha</Text><Card><Input label="Nova senha" value={password} onChangeText={setPassword} secureTextEntry /><Input label="Confirmar senha" value={confirm} onChangeText={setConfirm} secureTextEntry /><Button loading={loading} onPress={() => void submit()}>Salvar senha</Button></Card></Screen>;
}
