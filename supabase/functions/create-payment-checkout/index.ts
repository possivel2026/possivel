...
Deno.serve(async (req) => {
  ...
  const { data: payment, error: paymentError } = await db.from('payments')
    ...
    .eq('id', paymentId).eq('payer_id', user.id).maybeSingle();
  ...
  const preference = {
    items: [{ id: String(payment.id), title: payment.purpose || 'Pagamento Possível', quantity: 1, currency_id: 'BRL', unit_price: Number(payment.amount) }],
  ...
  const response = await fetch('https://api.mercadopago.com/checkout/preferences', {
    method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(preference),
  });
  if (!response.ok) return json({ error: await response.text() }, 502);
  const data = await response.json();
  await db.from('payments').update({ provider: 'mercadopago', provider_reference: String(data.id), updated_at: new Date().toISOString() }).eq('id', payment.id);
